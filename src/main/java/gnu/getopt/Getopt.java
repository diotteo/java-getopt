...
/**
  ...
  * @author Olivier Diotte (olivier@diotte.ca)
  *
  * @see LongOpt
  */
public class Getopt extends Object {
	...
	// array of LongOpt objects which describes the valid long options.
	protected LongOpt[] long_options;

	// whether or not we are parsing only long args
	protected boolean long_only;

	// index into the long_options array of the long option found
	protected int longind;

	// whether or not we operate in strict POSIX compliance
	protected boolean posixly_correct;

	// whether or not checkLongOption() did all necessary processing for the current option
	...
	public Getopt(String progname, String[] argv, String optstring,
		...
		if (optstring.charAt(0) == '-') {
			...
			ordering = Ordering.PERMUTE; // The normal default case
		}
	}
		...
		if (nameend != nextchar.length()) {
			if (pfound.has_arg != LongOpt.NO_ARGUMENT) {
				...
				if (opterr) {
					...
					if (argv[optind - 1].startsWith("--")) {
						...
						System.err.println(MessageFormat.format(
								_messages.getString("getopt.arguments1"), msgArgs));
					// +option or -option
					} else {
						Object[] msgArgs = {
								progname,
						...
						System.err.println(MessageFormat.format(
								_messages.getString("getopt.arguments2"), msgArgs));
					}
				}
				nextchar = "";
				optopt = pfound.val;
			...
			if (optind < argv.length) {
				...
				if (opterr) {
					...
					System.err.println(MessageFormat.format(
							_messages.getString("getopt.requires"), msgArgs));
				}
	...
	public int getopt() {
		...
		if ((nextchar == null) || (nextchar.equals(""))) {
			...
			if (argv[optind].equals("") || (argv[optind].charAt(0) != '-') ||
					argv[optind].equals("-")) {
				if (ordering == Ordering.REQUIRE_ORDER) {
					return -1;
				}
			...
			if (!long_only || argv[optind].startsWith("--")
				...
				if (opterr) {
					if (argv[optind].startsWith("--")) {
						...
						System.err.println(MessageFormat.format(
								_messages.getString("getopt.unrecognized"), msgArgs));
					} else {
						Object[] msgArgs = {
								progname,
						...
						System.err.println(MessageFormat.format(
								_messages.getString("getopt.unrecognized2"), msgArgs));
					}
				}
